...
def main():
    ...
    if representation == 'bag of words':
        ...
        #TODO: get more in test
        if not os.path.exists('train_image_feats_bof.npy'):
            train_image_feats = get_bags_of_words(train_image_paths, feature)
            np.save('train_image_feats_bof', train_image_feats)
        else:
            train_image_feats = np.load('train_image_feats_bof.npy')

        if not os.path.exists('test_image_feats_bof.npy'):
            test_image_feats = get_bags_of_words(test_image_paths, feature)
            np.save('test_image_feats_bof', test_image_feats)
        else:
            test_image_feats = np.load('test_image_feats_bof.npy')

    elif representation == 'spatial_pyramid_feats':

        if not os.path.exists('train_image_feats_sp.npy'):
            train_image_feats = get_spatial_pyramid_feats(train_image_paths, max_level, feature)
            np.save('train_image_feats_sp', train_image_feats)
        else:
            train_image_feats = np.load('train_image_feats_sp.npy')

        if not os.path.exists('test_image_feats_sp.npy'):
            test_image_feats = get_spatial_pyramid_feats(test_image_paths, max_level, feature)
            np.save('test_image_feats_sp', test_image_feats)
        else:
            test_image_feats = np.load('test_image_feats_sp.npy')
